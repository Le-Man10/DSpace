...
ALTER TABLE requestitem ADD COLUMN IF NOT EXISTS item_id UUID REFERENCES Item(uuid);
...
CREATE INDEX IF NOT EXISTS requestitem_item on requestitem(item_id);

ALTER TABLE requestitem RENAME COLUMN bitstream_id to bitstream_legacy_id;
ALTER TABLE requestitem ADD COLUMN IF NOT EXISTS bitstream_id UUID REFERENCES Bitstream(uuid);
...
CREATE INDEX IF NOT EXISTS requestitem_bitstream on requestitem(bitstream_id);